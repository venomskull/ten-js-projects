...
async function getMealById(id) {
    const getMealById = await fetch('https://www.themealdb.com/api/json/v1/1/lookup.php?i=' + id)
        .then(res => res.json())
        .then(res => res.meals[0]);
...
async function getMealBySearch(term) {
    const getMealBySearch = await fetch('https://www.themealdb.com/api/json/v1/1/search.php?s=' + term)
        .then(res => res.json())
        .then(res => res.meals);

    return getMealBySearch;
}
...
async function fetchFavMeals() {
    ...
    for (let i = 0; i < mealIds.length; i++) {
        var mealData = await getMealById(mealIds[i]);
        addMealFav(mealData);
    }
}
...
function addMealFav(mealData) {
    ...
    // const btn = document.querySelector('#fav-meals .clear');
...
searchBtn.addEventListener('click', async function () {
    const meals = await getMealBySearch(searchTerm.value);
    mealsEl.innerHTML = '';
    // searchTerm.value = '';
